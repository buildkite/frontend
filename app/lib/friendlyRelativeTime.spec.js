...
describe('friendlyRelativeTime', () => {
  BOOL_FIXTURES.forEach((seconds) => {
    BOOL_FIXTURES.forEach((capitalized) => {
      describe(`when supplied with \`{ seconds: ${seconds}, capitalized: ${capitalized} }\``, () => {
        DATE_FIXTURES.forEach((date) => {
          it(`renders a correct time in the past or present`, () => {
            MockDate.set(date);
            expect(friendlyRelativeTime(DATE_FIXTURES[0], {seconds, capitalized})).toMatchSnapshot();
            MockDate.reset();
          });

          it(`renders a correct time in the future`, () => {
            MockDate.set(DATE_FIXTURES[0]);
            expect(friendlyRelativeTime(date, {seconds, capitalized})).toMatchSnapshot();
            MockDate.reset();
          });
        });
      });
